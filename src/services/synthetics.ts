...
const getUserSyntecticsAccounts = async (token: string): Promise<SyntheticsAccount[]> => {
...
const getAccountStatus = async (token: string): Promise<AccountStatus> => {
    ...
    let response: AccountStatus = "Pending";
    try {
...
const activeSynteticsAccount = async (token: string, transaction: string): Promise<SyntheticsAccount> => {
    const axiosClient = getAxiosClient("evoxAPI");
    const config = {
        headers: {
            "Content-Type": "application/json",
    ...
    try {
        const { data } = await axiosClient.patch(`/synthetic/active/${transaction}`, {}, config);
        response = data;
    } catch (error: any) {
